...
import os
from database import db # db 객체 임포트
from api.item.controller import item_bp
from api.basics import basics_bp
from api.chatbot import ai_bp
import logging
...
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False # 이벤트 추적 비활성화
db.init_app(app) # db 객체를 애플리케이션과 연결
...
# ai_bp Blueprint를 애플리케이션에 등록
app.register_blueprint(ai_bp)

with app.app_context():
    db.create_all() # 데이터베이스 테이블 생성 (개발 시 유용)
